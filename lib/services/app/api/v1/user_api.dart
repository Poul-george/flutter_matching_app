...
//       print('User fetch: ${response.body}');
//       return user;
//     }
//   }).catchError((error) {
//     // HTTPリクエストが失敗した場合の処理
//     print('User fetch: ${error.toString()} ${error.hashCode}');
...
Future<User?> fetchUsers({
  ...
  try {
    final response = await http.get(
      Uri.parse('${config.apiHost}/api/v1/users/detail$data'),
      headers: {'Origin': config.origin},
    );

    if (response.statusCode == 200) {
      final jsonList = json.decode(response.body) as Map<String, dynamic>;
      User? user = User.fromJson(jsonList["item"]);
      print('User fetch: ${response.body}');
      return user;
    } else {
      print('User fetch failed: ${response.statusCode}');
      return null;
    }
  } catch (error) {
    print('User fetch failed error: $error');
    return null;
  }
}
